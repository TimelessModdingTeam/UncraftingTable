        ...
        if(inventory == calculInput)
        {
            if(calculInput.getStackInSlot(0) == null)
            {
                ...
                if(uncraftIn.getStackInSlot(0) == null)
                {
                    ...
                    result = r;
                    type = State.READY;
                    xp = -UncraftingTable.standardLevel;
                }
                return;
            }
            else if(uncraftIn.getStackInSlot(0) == null)
            {
                ...
                if(!MinecraftForge.EVENT_BUS.post(event))
                {
                    ...
                    if(UncraftingTable.instance.uncraftMethod == 0)
                    {
                        xp = 0;
                    }
                    else if(UncraftingTable.instance.uncraftMethod == 1)
                    {
        ...
        else if(inventory == uncraftIn)
        {
            ...
            if(!MinecraftForge.EVENT_BUS.post(event))
            {
                ...
                while(uncraftIn.getStackInSlot(0) != null && nbrStacks <= uncraftIn.getStackInSlot(0).stackSize)
                {
                    ...
                    if(lvl < UncraftingTable.standardLevel + xp && !player.capabilities.isCreativeMode)
                    {
                    ...
                    else if(lvl >= UncraftingTable.standardLevel + xp && !player.capabilities.isCreativeMode)
                    {
                        player.experienceLevel -= UncraftingTable.standardLevel + xp;
                    }
                    ...
                    {
                        event.getPlayer().addStat(UncraftingTable.instance.uncraftedItemsStat, event.getRequiredNumber());
                        event.getPlayer().triggerAchievement(UncraftingTable.instance.uncraftAny);
                    }
